...
const STATUS_METADATA: Record<JuryCase["status"], { label: string; badgeClass: string }> = {
  active: {
...
const JURY_CASES: JuryCase[] = [
  ...
  },
  {
    ...
    sellerResponse: "El modelo enviado es equivalente y cumple con las especificaciones indicadas en la publicación.",
    sellerEvidence: [
  ...
  },
  {
    ...
    sellerResponse: "Nunca detectamos pixeles muertos. Es posible que se hayan generado durante el envío.",
    sellerEvidence: [
  ...
  },
  {
    ...
    sellerResponse: "Se envió un cargador certificado compatible. El estado del teclado corresponde al uso descrito.",
    sellerEvidence: [
...
export default function ReviewsPage() {
  ...
  const [selectedCaseId, setSelectedCaseId] = useState<string>(JURY_CASES[0]?.id ?? "")
  const [caseNotes, setCaseNotes] = useState<Record<string, string>>(() =>
    Object.fromEntries(JURY_CASES.map((juryCase) => [juryCase.id, ""])),
  )
  const [userVotes, setUserVotes] = useState<Record<string, "buyer" | "seller" | null>>(() =>
    Object.fromEntries(JURY_CASES.map((juryCase) => [juryCase.id, null])),
  )
  const [voteBoard, setVoteBoard] = useState<Record<
    string,
    { buyer: number; seller: number; remaining: number }
  >>(() =>
    Object.fromEntries(
      JURY_CASES.map((juryCase) => [juryCase.id, { ...juryCase.votes }]),
    ),
  )
  const [feedback, setFeedback] = useState<{ type: "buyer" | "seller"; caseId: string } | null>(null)

  const selectedCase = useMemo(() => {
    return JURY_CASES.find((juryCase) => juryCase.id === selectedCaseId) ?? JURY_CASES[0]
  }, [selectedCaseId])

  const sidebarCases = useMemo(() => {
    const priorityOrder: JuryCase["status"][] = ["active", "pending", "review"]
    return [...JURY_CASES].sort((a, b) => priorityOrder.indexOf(a.status) - priorityOrder.indexOf(b.status))
  }, [])
  ...
  const voteTendency = useMemo(() => {
    if (!selectedCase) return { label: "Sin datos", className: "bg-slate-500/20 text-slate-300" }

    if (selectedVoteBoard.buyer === selectedVoteBoard.seller) {
      return { label: "Empate temporal", className: "bg-amber-500/20 text-amber-400" }
    }

    return selectedVoteBoard.buyer > selectedVoteBoard.seller
      ? { label: "✅ Tendencia: Disputa Exitosa", className: "bg-green-500/20 text-green-400" }
      : { label: "❌ Tendencia: Disputa Fallida", className: "bg-red-500/20 text-red-400" }
  ...
  const headingGradient = isDarkMode ? "from-cyan-400 to-blue-400" : "from-blue-600 to-cyan-600"
      ...
      <div className="container mx-auto space-y-8 px-4 sm:px-6 lg:px-10">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className={`text-2xl font-bold sm:text-3xl bg-gradient-to-r ${headingGradient} bg-clip-text text-transparent`}>
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
          ...
          <div className="space-y-4 lg:col-span-1">
            <div className={`${sectionClass} p-4`}>
              <h3 className={`mb-4 font-semibold ${titleTextClass}`}>Casos para Revisar</h3>
              <div className="space-y-2">
                {sidebarCases.map((juryCase) => {
                  ...
                  const highlight = isSelected
                    ...
                    : ""
                  const activeClasses = juryCase.status === "active"
                  ...
                  return (
                    ...
                    >
                      <span className="block font-semibold">Caso #{juryCase.id}</span>
                      <span className={`block text-xs ${isDarkMode ? "text-slate-300" : "text-slate-600"}`}>
            ...
            {/* Stats */}
            <div className={`${sectionClass} p-4`}>
              <h3 className={`mb-4 font-semibold ${titleTextClass}`}>Mis Estadísticas</h3>
          ...
          <div className="space-y-6 lg:col-span-2">
            ...
            <div className={`${sectionClass} p-6`}>
              <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <div>
                  <p className={`text-sm font-medium ${subtitleTextClass}`}>Caso #{selectedCase?.id}</p>
                  <h3 className={`text-xl font-semibold ${titleTextClass}`}>{selectedCase?.title}</h3>
                </div>
                <span className={`w-max rounded-full px-3 py-1 text-sm font-medium ${currentStatus.badgeClass}`}>
                  {currentStatus.label}
                </span>
              </div>
              ...
              <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
                ...
                <div className="space-y-4">
                  <div className={`${cardClass} p-4`}>
                    <h4 className={`mb-2 font-semibold ${titleTextClass}`}>Producto en Disputa</h4>
                    <div className="flex items-center gap-3">
                      ...
                      />
                      <div>
                        <p className={`font-medium ${titleTextClass}`}>{selectedCase?.productName}</p>
                        <p className="font-bold text-[#ff9800]">{selectedCase?.price}</p>
                      </div>
                    </div>
                  </div>

                  <div className={`${cardClass} p-4`}>
                    <h4 className={`mb-2 font-semibold ${titleTextClass}`}>Partes Involucradas</h4>
                    <div className="space-y-2">
                      <div className="flex justify-between">
                        <span className={subtitleTextClass}>Comprador:</span>
                        <span className={titleTextClass}>{selectedCase?.buyerHandle}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className={subtitleTextClass}>Vendedor:</span>
                        <span className={titleTextClass}>{selectedCase?.sellerHandle}</span>
                      </div>
                    </div>
                  </div>

                  <div className={`${cardClass} p-4`}>
                    <h4 className={`mb-2 font-semibold ${titleTextClass}`}>Motivo de la Disputa</h4>
                ...
                <div className="space-y-4">
                  <div className={`${cardClass} p-4`}>
                    <h4 className={`mb-3 font-semibold ${titleTextClass}`}>Evidencia del Comprador</h4>
                    <div className="grid grid-cols-2 gap-2">
                      {selectedCase?.buyerEvidence.map((item) => (
                        <figure key={item.caption} className="space-y-1">
                          ...
                          />
                          <figcaption className={`text-xs ${subtitleTextClass}`}>{item.caption}</figcaption>
                        </figure>
                      ))}
                    </div>
                  </div>
                  ...
                  <div className={`${cardClass} p-4`}>
                    <h4 className={`mb-3 font-semibold ${titleTextClass}`}>Respuesta del Vendedor</h4>
              ...
              <div className={`${cardClass} mt-6 p-4`}>
                <h4 className={`mb-4 font-semibold ${titleTextClass}`}>Tu Veredicto como Jurado</h4>
                ...
                <div className="mt-4">
                  <label className={`mb-2 block text-sm ${subtitleTextClass}`}>Comentario adicional (opcional):</label>
                  <textarea
                    placeholder="Explica tu decisión para ayudar a otros jurados..."
                    value={caseNotes[selectedCase?.id ?? ""] ?? ""}
                    onChange={(event) => handleNoteChange(selectedCase?.id ?? "", event.target.value)}
              ...
              <div className={`${cardClass} mt-4 p-4`}>
                <h5 className={`mb-3 font-medium ${titleTextClass}`}>Estado Actual de Votación</h5>
                <div className="grid grid-cols-2 gap-4">
                  <div className="text-center">
                    <div className="text-2xl font-bold text-green-400">{selectedVoteBoard.buyer}</div>
                    <div className={`text-sm ${subtitleTextClass}`}>Disputa Exitosa</div>
                  </div>
                  <div className="text-center">
                    <div className="text-2xl font-bold text-red-400">{selectedVoteBoard.seller}</div>
                    <div className={`text-sm ${subtitleTextClass}`}>Disputa Fallida</div>
                  </div>
                </div>
                <div className="mt-2 text-center">
                  <span className={`text-xs ${mutedTextClass}`}>
                    Faltan {selectedVoteBoard.remaining} votos para cerrar el caso
                  </span>
                </div>
                <div className="mt-3 text-center">
                  <div className={`inline-block rounded-full px-3 py-1 text-sm ${voteTendency.className}`}>
                    {voteTendency.label}
                  </div>
                </div>
              </div>
            ...
            <div className={`${sectionClass} p-4`}>
              <h3 className={`mb-4 font-semibold ${titleTextClass}`}>Casos Pendientes de Revisión</h3>
              <div className="space-y-3">
                {queueCases.map((juryCase) => {
                  ...
                  return (
                    ...
                    >
                      <div>
                        <p className={`font-medium ${titleTextClass}`}>Caso #{juryCase.id} - {juryCase.productName}</p>
                        <p className={`text-sm ${subtitleTextClass}`}>{juryCase.queueTime}</p>
                      </div>
                      <Button
                        className={`text-sm text-white ${
                          isSelected ? "bg-[#00bcd4] hover:bg-[#00a0b3]" : "bg-[#ff9800] hover:bg-[#f57c00]"
                        }`}
                        onClick={() => handleSelectCase(juryCase.id)}
                      >
