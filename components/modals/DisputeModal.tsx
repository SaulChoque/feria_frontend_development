      ...
      <div className="space-y-6">
        <header className="space-y-2 text-center sm:text-left">
          ...
          <p className="text-sm text-white/70">
            Proporciona la información necesaria para que nuestro equipo pueda revisar tu caso.
          </p>
        </header>
          ...
          <div className="rounded-2xl border border-white/20 bg-white/5 p-4">
            <div className="flex flex-col items-center gap-4 text-center sm:flex-row sm:items-center sm:text-left">
              ...
              <div className="space-y-1">
                <h3 className="text-lg font-semibold text-white sm:text-xl">{disputeProduct.name}</h3>
                <p className="text-[#ff9800] font-bold">${disputeProduct.price} USDC</p>
                <p className="text-sm text-white/60">Vendedor: {disputeProduct.seller}</p>
              </div>
            </div>
          </div>
        )}
        ...
        <div className="space-y-6">
          <div className="space-y-3">
            <label className="block text-sm font-medium text-white">Motivo de la Disputa</label>
            <select
              value={disputeReason}
          ...
          <div className="space-y-3">
            <label className="block text-sm font-medium text-white">Descripción Detallada</label>
            <textarea
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-3">
              ...
              {disputeImages.length < 5 && (
                <label className="flex h-24 w-full flex-col items-center justify-center rounded-lg border-2 border-dashed border-white/30 text-white/60 transition-colors hover:border-[#00bcd4]">
                  ...
                  <input type="file" accept="image/*" onChange={addDisputeImage} className="hidden" />
                </label>
              )}
            </div>
            <p className="text-xs text-white/60">Puedes agregar hasta 5 imágenes como evidencia ({disputeImages.length}/5)</p>
          </div>
        </div>
