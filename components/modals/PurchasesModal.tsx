    ...
    >
      <div className="space-y-6">
        <header className="space-y-2 text-center sm:text-left">
          ...
          <div>
            <h2 className="text-3xl font-bold text-white">Historial de compras</h2>
            <p className="mt-2 text-sm text-white/70">
              Revisa el estado de tus pedidos y gestiona disputas o apelaciones desde un solo lugar.
            </p>
          </div>
        </header>
                ...
                {userPurchases.map((product) => (
                  ...
                  >
                    <div className="flex items-center justify-between">
                      ...
                      <div className="ml-3">
                        {product.purchaseStatus === "pendiente" && <div className="h-3 w-3 rounded-full bg-yellow-400" />}
                        {product.purchaseStatus === "completado" && <div className="h-3 w-3 rounded-full bg-green-500" />}
                        {product.purchaseStatus === "disputa" && <div className="h-3 w-3 rounded-full bg-red-500" />}
                        {product.purchaseStatus === "en revision" && <div className="h-3 w-3 rounded-full bg-blue-400" />}
                      </div>
                    </div>
                  </button>
                ))}
            ...
            {selectedPurchaseProduct ? (
              <div className="h-full rounded-2xl border border-white/20 bg-white/5 p-6">
                <div className="mb-6 flex flex-col gap-6 sm:flex-row">
                  ...
                  <div className="flex-1 space-y-3">
                    <div>
                      <h3 className="text-2xl font-bold text-white">{selectedPurchaseProduct.name}</h3>
                      <p className="text-3xl font-bold text-[#ff9800]">${selectedPurchaseProduct.price} USDC</p>
                    </div>
                    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                      <div className="rounded-lg border border-white/20 bg-white/10 p-3">
                        <p className="text-xs text-white/60">Vendedor</p>
                        <p className="text-sm font-medium text-white">{selectedPurchaseProduct.seller}</p>
                      </div>
                      <div className="rounded-lg border border-white/20 bg-white/10 p-3">
                ...
                <div className="space-y-4">
                  {selectedPurchaseProduct.purchaseStatus === "pendiente" && (
                    <div className="rounded-2xl border border-white/20 bg-white/10 p-6">
                      <h4 className="mb-4 text-lg font-semibold text-white">Acciones de Compra</h4>
                  ...
                  {selectedPurchaseProduct.purchaseStatus === "completado" && (
                    <div className="rounded-2xl border border-green-500/30 bg-green-500/20 p-6 text-center text-white">
                      ...
                      <p className="mt-3 text-white/70">¡Gracias por tu compra! Esperamos que disfrutes tu producto.</p>
                    </div>
                  )}
                  ...
                  {selectedPurchaseProduct.purchaseStatus === "disputa" && (
                    <div className="rounded-2xl border border-red-500/30 bg-red-500/20 p-6 text-center text-white">
                      ...
                      <p className="mt-3 text-white/70">Tu disputa ha sido registrada. Un moderador revisará el caso pronto.</p>
                    </div>
                  )}
                    ...
                    <div className="rounded-2xl border border-white/20 bg-white/10 p-6">
                      <div className="mb-6 text-center">
                        ...
                        <p className="text-white/80">
                          El vendedor ha apelado tu disputa. Puedes revisar su respuesta y decidir cómo proceder.
                        </p>
                      </div>
                      <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                        <Button
              ...
              <div className="flex h-full items-center justify-center rounded-2xl border border-white/20 bg-white/5 p-12 text-center">
                <div>
                  ...
                  <p className="text-lg text-white/60">Selecciona una compra para ver los detalles</p>
                </div>
              </div>
            )}
          </div>
