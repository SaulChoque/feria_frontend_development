				...
				<div className="space-y-6">
					<header className="space-y-2 text-center sm:text-left">
						...
						<p className="text-sm text-white/70">
							Revisa la evidencia del comprador y decide si deseas aceptar la
							disputa o presentar una apelación.
						</p>
					</header>
						...
						<section className="space-y-4 rounded-2xl border border-white/20 bg-white/5 p-6">
							<div className="flex flex-col items-center gap-4 text-center sm:flex-row sm:items-center sm:text-left">
								...
								<div>
									<h3 className="text-lg font-semibold text-white">
										{reviewingDisputeProduct.name}
									</h3>
									<p className="text-[#ff9800] font-bold">
										${reviewingDisputeProduct.price} USDC
									</p>
									<p className="text-sm text-white/60">
										Comprador:{" "}
										{getDisputeInfo(reviewingDisputeProduct.name).comprador}
									</p>
								</div>
							</div>
							...
							<div className="space-y-4">
								<div className="relative">
									<img
										src={
											getDisputeInfo(reviewingDisputeProduct.name).imagenes[
												currentImageIndex
											] || "/placeholder.svg"
										}
								...
								<div className="flex gap-2 overflow-x-auto">
									{getDisputeInfo(reviewingDisputeProduct.name).imagenes.map(
										(image, index) => (
											<button
												key={index}
						...
						<section className="space-y-4 rounded-2xl border border-white/20 bg-white/5 p-6">
							<div className="space-y-2">
								...
								<p className="text-sm text-white/80">
									{getDisputeInfo(reviewingDisputeProduct.name).motivo}
								</p>
							</div>
							<div className="space-y-2">
