				...
				<div className="space-y-6">
					<header className="space-y-2 text-center sm:text-left">
						...
						<p className="text-sm text-white/70">
							Revisa la evidencia proporcionada por el vendedor y decide el
					...
					<div className="space-y-6 rounded-2xl border border-white/20 bg-white/5 p-6">
						<div className="flex flex-col items-center gap-4 text-center sm:flex-row sm:items-center sm:text-left">
							...
							<div>
								<h3 className="text-lg font-semibold text-white">
									{appealReviewProduct.name}
								</h3>
								<p className="text-[#ff9800] font-bold">
									${appealReviewProduct.price} USDC
								</p>
								<p className="text-sm text-white/60">
									Vendedor:{" "}
									{getSellerAppealInfo(appealReviewProduct.name).vendedor}
								</p>
							</div>
						</div>
						...
						<div className="space-y-4 rounded-2xl border border-yellow-500/30 bg-yellow-500/10 p-6 text-center">
							...
							<p className="text-white/80">
								El vendedor ha apelado tu disputa y ha proporcionado evidencia
								adicional. Revisa la información y decide cómo proceder.
							</p>
						</div>
						...
						<div className="grid gap-6 md:grid-cols-2">
							<section className="space-y-4 rounded-2xl border border-white/20 bg-white/5 p-6">
								...
								<div className="relative">
									<img
										src={
											getSellerAppealInfo(appealReviewProduct.name)
												.imagenesApelacion[0] || "/placeholder.svg"
										}
								...
								<div className="flex gap-2 overflow-x-auto">
									{getSellerAppealInfo(
										appealReviewProduct.name
									).imagenesApelacion.map((image: string, index: number) => (
										<div
											key={index}
											className="h-16 w-16 flex-shrink-0 rounded-lg border-2 border-white/20 overflow-hidden"
										>
											<img
												src={image}
							...
							<section className="space-y-4 rounded-2xl border border-white/20 bg-white/5 p-6">
								<div className="space-y-2">
									...
									<p className="text-sm text-white/80">
										...
										{getSellerAppealInfo(appealReviewProduct.name)
											.motivoApelacion === "evidencia-falsa" &&
											"Evidencia del comprador es falsa"}
										{getSellerAppealInfo(appealReviewProduct.name)
											.motivoApelacion === "descripcion-correcta" &&
											"Producto coincide con la descripción"}
										{getSellerAppealInfo(appealReviewProduct.name)
											.motivoApelacion === "comunicacion-previa" &&
											"Había comunicación previa con el comprador"}
										{getSellerAppealInfo(appealReviewProduct.name)
											.motivoApelacion === "otro" && "Otro"}
									</p>
								</div>
								<div className="space-y-2">
									<h4 className="text-lg font-semibold text-white">
										Explicación del Vendedor
									</h4>
							...
							<div className="grid grid-cols-1 gap-4 md:grid-cols-2">
								...
								>
									...
									<div className="mt-1 text-xs opacity-80">
										Aceptar la respuesta del vendedor
									</div>
								</Button>
								<Button
								...
								>
									💬 Seguir Discutiendo
									<div className="mt-1 text-xs opacity-80">
										Solicitar mediación de un moderador
									</div>
								</Button>
							</div>
						</section>
